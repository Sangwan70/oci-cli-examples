...
echo
oci compute shape list -c $C --output table --query "sort_by(data[?contains("shape",'VM.Standard2.1')],&\"shape\") [*].{ShapeName:shape,Memory:\"memory-in-gbs\",CPUcores:ocpus}"
RED='\033[0;31m'
GREEN='\033[0;32m'
BLUE='\033[1;34m'
...
read -p "Enter the Shape name you wish to create [VM.Standard2.1: " shape
shape=${shape:-"VM.Standard2.1"}

read -p "Enter the Path of your ssh key [~/.ssh/id_rsa.pub]: " public_key
public_key=${public_key:-~/.ssh/id_rsa.pub}  # this is a default path
...
do
  case $opt in
        "Oracle-Linux")
          oci compute image list -c $C --query "reverse(sort_by(data[?contains(\"display-name\",'Oracle-Linux')],&\"time-created\")) |[0:1].{ImageName:\"display-name\", OCID:id, OS:\"operating-system\", Size:\"size-in-mbs\",time:\"time-created\"}" --output table
          ...
          ;;
        "CentOS")
          oci compute image list -c $C --query "reverse(sort_by(data[?contains(\"display-name\",'CentOS')],&\"time-created\")) |[0:1].{ImageName:\"display-name\", OCID:id, OS:\"operating-system\", Size:\"size-in-mbs\",time:\"time-created\"}" --output table
        ...
        "Oracle Autonomus Linux")
          oci compute image list -c $C --query "reverse(sort_by(data[?contains(\"display-name\",'Oracle-Autonomous-Linux')],&\"time-created\")) |[0:1].{ImageName:\"display-name\", OCID:id, OS:\"operating-system\", Size:\"size-in-mbs\",time:\"time-created\"}" --output table
          ...
          ;;
        "Ubuntu")
          oci compute image list -c $C --query "reverse(sort_by(data[?contains(\"display-name\",'Canonical-Ubuntu')],&\"time-created\")) |[0:1].{ImageName:\"display-name\", OCID:id, OS:\"operating-system\", Size:\"size-in-mbs\",time:\"time-created\"}" --output table
          ...
          ;;
        "Windows")
          oci compute image list -c $C --query "reverse(sort_by(data[?contains(\"display-name\",'Windows')],&\"time-created\")) |[0:1].{ImageName:\"display-name\", OCID:id, OS:\"operating-system\", Size:\"size-in-mbs\",time:\"time-created\"}" --output table
...
echo "********** Network ***********"
# ocid_img=$(oci compute image list -c $C --operating-system "Oracle Linux" --operating-system-version "7.9" --shape $shape --query 'data[0].id' --raw-output) 
while true; do
